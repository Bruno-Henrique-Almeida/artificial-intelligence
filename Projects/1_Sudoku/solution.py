...
diagonal_unit_main = [r + c for r, c in zip(rows, cols)]
diagonal_unit_secondary = [r + c for r, c in zip(rows, cols[::-1])]

row_units = [cross(r, cols) for r in rows]
column_units = [cross(rows, c) for c in cols]
square_units = [cross(rs, cs) for rs in ("ABC","DEF","GHI") for cs in ("123","456","789")]
diagonal_units = [diagonal_unit_main, diagonal_unit_secondary]
...
def naked_twins(values: Dict[str, str]) -> Union[Dict[str, str], bool]:
...
def reduce_puzzle(values: Dict[str, str]) -> Union[Dict[str, str], bool]:
